document.addEventListener("DOMContentLoaded", function() {
    ...
    window.openCategory = function(categoryName, type, prefix, imagesCount = 12) {
        ...
        if(overlayDesc) {
            ...
            // Единый справочник: Категория -> Посадочная страница (Landing Page)
            // При добавлении новых страниц, кнопки в каталоге появятся АВТОМАТИЧЕСКИ
            const categoryLinks = {
                "Алкогольная продукция": "alcohol-labels.html",
                "Косметика и парфюмерия": "cosmetics-labels.html",
                "Пищевая продукция": "food-labels.html",
                "Автохимия": "autochem-labels.html",
                "Бытовая химия": "household-labels.html",
                "Термоусадочная этикетка": "shrink-labels.html",
                "Честный Знак": "chestny-znak.html"
            };
            
            // Если для категории есть ссылка, рисуем красивую кнопку "ПОДРОБНЕЕ"
    ...
    // --- НОВАЯ УНИВЕРСАЛЬНАЯ ЛОГИКА: Автоматическое открытие каталога по URL ---
    const urlParamsMain = new URLSearchParams(window.location.search);
    const catalogParam = urlParamsMain.get('catalog');

    // Универсальный справочник для входящих ссылок вида ?catalog=...
    const catalogMapping = {
        'alcohol': { name: 'Алкогольная продукция', type: 'industry', prefix: 'alcohol' },
        'cosmetics': { name: 'Косметика и парфюмерия', type: 'industry', prefix: 'cosmetics' },
        'food': { name: 'Пищевая продукция', type: 'industry', prefix: 'food' },
        'autochem': { name: 'Автохимия', type: 'industry', prefix: 'autochem' },
        'household': { name: 'Бытовая химия', type: 'industry', prefix: 'household' },
        'shrink': { name: 'Термоусадочная этикетка', type: 'industry', prefix: 'shrink' },
        'chestny_znak': { name: 'Честный Знак', type: 'product', prefix: 'chestny_znak' }
    };

    if (catalogParam && catalogMapping[catalogParam]) {
        // ИСПРАВЛЕНИЕ: Сразу тихо прокручиваем фоновую страницу к блоку каталога
        const productsSection = document.getElementById('products');
        if (productsSection) {
            // Без плавной анимации, чтобы фон мгновенно "прыгнул" на нужное место
            productsSection.scrollIntoView({ behavior: 'auto' });
        }

        // Задержка нужна для того, чтобы DOM полностью загрузился
        setTimeout(() => {
            if (typeof window.openCategory === 'function') {
                const catData = catalogMapping[catalogParam];
                window.openCategory(catData.name, catData.type, catData.prefix, 12);
            }
        }, 500);
    }
    // --- КОНЕЦ НОВОЙ ЛОГИКИ ---
